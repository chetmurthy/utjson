...
let convert1 cc ~with_predefined ~verbose infile outfile =
...
let convert_to_utj with_predefined verbose filepath destdir outputfile files =
  ...
  | ("", [infile], outfile) when outfile <> "-" ->
    ...
    convert1 cc ~with_predefined ~verbose infile outfile
...
let convert_cmd =
  ...
  in
  Term.(const convert_to_utj $ with_predefined $ verbose $ filepath $ destdir $outputfile $ files),
...
if not !Sys.interactive then
  Term.(exit @@ eval_choice help_cmd [help_cmd; convert_cmd])
;;

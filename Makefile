...
OBJ=ututil.cmo utypes.cmo utlexing.cmo utparse0.cmo utprint.cmo utconv.cmo uttypecheck.cmo uteval.cmo utvalid0.cmo
OML=ututil.ml utypes.ml syntax_test.ml schemastore_test.ml typing_test.ml utconv.ml uttypecheck.ml uteval.ml utvalid0.ml
LEXML=utlexing.ml
...
depend::
	$(OCAMLFIND) ocamldep -package $(PACKAGES) -syntax camlp5o \
		$(OML) \
		 > .depend.NEW
	$(OCAMLFIND) ocamldep -package sedlex.ppx \
		$(LEXML) \
		 >> .depend.NEW
